...
JOBID="${1:?Usage: run_cmsdriver_mc.sh JOBID [YAML_HEADER_SAMPLE] [INPUT_LIST]}"
...
if [ "$SAMPLE_DIR" = "unclassified" ]; then
    INPUT_LIST="${3:-miniAODSIM_chunk_${JOBID}.txt}"
else
    INPUT_LIST="${3:-miniAODSIM_chunk_${SAMPLE_DIR}_${JOBID}.txt}"
fi
